				...
				void *user_data)
{
	...
	switch (entry_type) {
	case CONNLINE_DBUS_ENTRY_BASIC:
		connline_dbus_append_basic(&entry, key_name,
						dbus_type, user_data);
		break;
	case CONNLINE_DBUS_ENTRY_ARRAY:
		connline_dbus_append_array(&entry, key_name, dbus_type,
					property_function, user_data);
		break;
	case CONNLINE_DBUS_ENTRY_FIXED_ARRAY:
		connline_dbus_append_fixed_array(&entry, key_name,
					dbus_type, length, user_data);
		break;
	case CONNLINE_DBUS_ENTRY_DICT:
		connline_dbus_append_dict(&entry, key_name,
					property_function, user_data);
		break;
	default:
		break;
...
static int connline_dbus_get(DBusMessageIter *iter,
					enum connline_dbus_entry entry_type,
					int dbus_type,
					int *length,
					void *destination)
{
	switch (entry_type) {
		case CONNLINE_DBUS_ENTRY_BASIC:
			return connline_dbus_get_basic(iter,
						dbus_type, destination);
		case CONNLINE_DBUS_ENTRY_ARRAY:
			return connline_dbus_get_array(iter,
					dbus_type, length, destination);
		case CONNLINE_DBUS_ENTRY_FIXED_ARRAY:
			return connline_dbus_get_fixed_array(iter,
							length, destination);
		case CONNLINE_DBUS_ENTRY_DICT:
			if (destination == NULL)
				return -EINVAL;
					...
					void *destination)
{
	...
	while (arg_type != DBUS_TYPE_INVALID) {
		...
		return connline_dbus_get(&struct_entry, entry_type,
					dbus_type, length, destination);
	}
					...
					void *user_data)
{
	...
	dbus_bus_add_match(dbus_cnx, rule, &error);
	if (dbus_error_is_set(&error) == TRUE) {
		dbus_error_free(&error);
		return -ENOMEM;
	}

	if (dbus_connection_add_filter(dbus_cnx,
					filter, user_data, NULL) == FALSE) {
		dbus_bus_remove_match(dbus_cnx, rule, NULL);
		return -ENOMEM;
	}
...

{
	...
	dbus_bus_remove_match(dbus_cnx, rule, &error);
	if (dbus_error_is_set(&error) == TRUE)
		dbus_error_free(&error);
